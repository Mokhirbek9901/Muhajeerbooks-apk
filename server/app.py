...
def admin_ai_book_research():
    ...
    instructions="""Research the exact book on the web. Verify bibliographic facts using reliable sources. Return ONLY valid compact JSON with keys: title, author, publisher, category, description, cover, confidence, notes. Write ONE short Uzbek bookstore description only, strictly 35-50 words. Base it on the verified content of the exact book. In 2-3 compact sentences, say what the book is about and make the reader curious to read it, without spoilers. Use natural sales-friendly Uzbek, not exaggerated advertising. Do not include prices, URLs, citations, copied jacket text, generic filler, or invented claims. A later request may intentionally produce a differently worded description, so do not return multiple variants. Do not guess uncertain fields; use empty strings. category must be exactly one of: Badiiy adabiyot; Biznes va moliya; Bolalar adabiyoti; Diniy-ma’rifiy; Islom tarixi; Jahon adabiyoti; Jamiyat va kommunikatsiya; O‘zbek adabiyoti; Oila va nikoh; Psixologiya va shaxsiy rivojlanish; Qur’on va islom ilmlari; Ta’lim va tillar; Tarix va biografiya; Texnologiya; Tibbiyot va jamiyat. cover must be Yumshoq muqova, Qattiq muqova, or empty. confidence is high/medium/low. notes briefly states uncertainty."""

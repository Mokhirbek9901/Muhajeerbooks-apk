...
new_orders = old_orders + '''
  Future<void> reloadQuietly() async {
    try {
      ...
      // Fon refreshda loading/xato ekraniga o'tmaymiz.
    }
  }
'''
...
new_customers = '''  void reload() {
  ...
  Future<void> reloadQuietly() async {
    try {
      ...
      // Fon refreshda mijozlar ro'yxatini yo'qotmaymiz.
    }
  }
...
# -------------------------
# AppState: in-app order notifications
# -------------------------
...
notice_methods = '''
  ...
  Future<void> _refreshCustomerOrderStatusesQuietly() async {
    ...
    try {
      ...
      // Internet vaqtincha uzilsa lokal holat saqlanadi.
    } finally {
...
# -------------------------
# Store UI: bell badge + notification center + instant in-app snackbar
# -------------------------
    ...
    if (latestNoticeId != null && latestNoticeId != _lastPresentedNoticeId) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        ScaffoldMessenger.of(context)
          ...
          ..showSnackBar(
            SnackBar(
              content: Text((notice['title'] ?? 'Buyurtma yangilandi').toString()),
...
print('Patched admin silent refresh + customer in-app order notices')
